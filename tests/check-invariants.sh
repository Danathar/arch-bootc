...
# ---------------------------------------------------------------------------
group "Read boundary on allowed Bash (.claude/settings.json: the entries the PreToolUse gate is built on)"
...
# `Bash(git log*)` and `Bash(git show*)`. Those rules are on different tools:
# `git diff <a> <b>` in its two-path mode prints any file this uid can open as
# `+` lines, and `--output=FILE` on any of the three writes over any file this
# uid can reach, and the deny rules on Read are never consulted for either. No
# permission pattern closes that, because patterns match a command prefix and
# flags may appear in any order. The PreToolUse hook the same file registers,
# .claude/hooks/gate-git-diff.sh, is handed the whole command and refuses those
# forms.
#
# The hook is *run*, not grepped, in tests/test-gate-git-diff.sh: against the
# spellings it refuses, the ordinary diffs it must leave unprompted, with jq
# off PATH, and with each of its rules disabled in turn. That is a behavioral
# suite -- it runs git against mktemp fixtures and takes minutes -- so it lives
# with the other tests/test-*.sh files that run-tests.sh executes and
# check-coverage.sh traces, and not in this static script. What stays here is
# the half that reads the tree: the settings.json entries the hook's rationale
# depends on, and that the hook the settings entry names exists and can run.
...
if ((settings_readable)); then
  ...
  # And these two, which are what make the --output write unprompted. They are
  ...
  if ((bash_hook_count > 0)); then
    ...
    fail "${CLAUDE_SETTINGS} declares a PreToolUse command hook on Bash" \
      "nothing is handed the command string, so every flag ordering is unprompted"
  fi
