...
# Plain bash tests under tests/. They need no root, no container runtime and no
...
#
# Run the shell tests, enforce per-script coverage floors, and assert the
# repository invariants. check-invariants.sh is static and reads only the
# checked-out tree, so it costs nothing to run alongside the suite -- and it is
# the only check that notices an invariant AGENTS.md calls load-bearing having
# quietly stopped being true.
#
# Run the shell tests, the coverage floors, and the repository invariant checks.
test:
    ./tests/check-coverage.sh
    ./tests/check-invariants.sh
...
lint:
    ...
    shellcheck system_files/usr/bin/ostree-pkg-diff system_files/usr/libexec/arch-bootc-prune-esp scripts/quickstart.sh scripts/pr-review-state.sh scripts/prune-package-versions.sh tests/run-tests.sh tests/check-coverage.sh tests/check-invariants.sh .claude/hooks/gate-git-diff.sh tests/tune-coverage.sh tests/test-gate-git-diff.sh tests/test-prune-esp.sh tests/test-homebrew-profile.sh tests/test-homebrew-shell-integration.sh tests/test-nightly-compliance.sh tests/test-ostree-pkg-diff.sh tests/test-ostree-pkg-diff-db.sh tests/test-pr-review-state.sh tests/test-prune-package-versions.sh tests/test-quickstart-baremetal.sh tests/e2e/test-quickstart-dry-run.sh
